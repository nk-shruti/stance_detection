...
def init_model(preload=None, declare=True, data=None):
	...
	if not declare and preload: return load_model(preload)
		...
		# aux_inp1 = Input(shape=(128,),name='aux_input_1')
		# aux_inp = Input(shape=(128,),name='aux_input')
		# x = concatenate()

		# aux_output = [128]*aux_output
...
def runner(epochs):
	...
	l = np.zeros((y_train.shape[0],4))
	v = np.zeros((y_val.shape[0],4))
	...
	labels2_train = np.asarray(labels2_train)
	labels2_val = np.asarray(labels2_val)
	...
	model.fit([x_train_1,x_train_2], [l,labels2_train], 
			class_weight = {'main_output':class_weights,'aux_output':class_weights_2},
			validation_data=([x_val_1,x_val_2 ], [v,labels2_val]),batch_size=32, epochs=20,
